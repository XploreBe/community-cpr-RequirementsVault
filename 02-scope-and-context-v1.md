...
- **Stakeholders:** Mohamed (BA/product owner for this pipeline), a 2-developer + QA + BA delivery team (per the architecture diagram's subtitle), 911/112 dispatchers and supervisors/admins (console users), volunteers (app users), and — indirectly — EMS services. No tension between stakeholders is recorded yet; none of the brief's content suggests conflicting stakeholder demands beyond the navigation-provider conflict logged as OQ-006.
...
| Fase 1 | volunteer-app | Alert response walking skeleton (no login, mocked data) | REQ-F-014, 015 (conditional), 024, 025, 026 (conditional), 027, 028, 029, 030 | Sign up, see a mocked alert, accept/decline, navigate, reference material, post-event check-in — no real push, no cert workflow yet. |
...
- **REQ-F-015** — select a tier at sign-up. **Conditional on OQ-001**: scoped using the brief's three provisional tier names (certified, healthcare professional, willing-but-untrained) as a placeholder list; final breakdown to be confirmed.
- **REQ-F-024, REQ-F-025** — accept/decline a (mocked, not push-delivered) incoming alert with one tap.
- **REQ-F-026** — navigate to the scene via a maps deep link with mocked coordinates. **Conditional on OQ-006**: scoped using Google Maps per the brief's text, since it is the more specific and less ambiguous of the two conflicting sources; the diagram's OpenStreetMap/MapLibre/OSRM reference is not implemented under this assumption and the conflict is not resolved by this choice.
...
Within each Phase 1 repo, the internal build order follows the "Suggested build order" in 03-product-backlog-v1.md (create before view/update/resolve, sign-up before alert-handling, module scaffold before endpoints).
...
| REQ-F-014, 015 (sign-up + tier) | Must | S | Medium | Depends on OQ-001 (tier breakdown) — scoped under a stated assumption, but could need rework if the final breakdown differs structurally. |
| REQ-F-024, 025 (accept/decline) | Must | S | Laag | Simple state toggle on mocked data. |
| REQ-F-026 (navigate) | Must | S | Medium | Depends on OQ-006 (map provider conflict) — scoped under an assumption (Google Maps); rework risk if the diagram's choice is confirmed instead. |
...
| REQ-F-001, REQ-N-005 (login/roles/MFA) | Must | M | Medium | Well-understood pattern, but three roles with undefined permission boundaries (OQ-011) add rework risk. |
| REQ-F-006, 031, REQ-N-017 (radius search, nearby-volunteer search, sub-second target) | Must | L | High | Geospatial database/indexing choice is an open question (OQ-003); performance target itself is not precisely defined (OQ-007). |
| REQ-F-007, 008, 009, 032 (tiered alert send + widening, configurable per country) | Must | L | High | Depends on unresolved tier breakdown (OQ-001) and country-abstraction approach (OQ-005); genuinely new logic. |
...
| REQ-F-021, 022, 033, 034 (real push send + DND handling + delivery tracking) | Must | L | High | Push reliability approach is an open question (OQ-004); DND-bypass feasibility is platform-dependent and unproven here. |
| REQ-F-016..020, 023 (cert upload/expiry/reminders, availability, background location + consent) | Must/Should | M | Medium | Several sub-flows, but each individually well understood; consent/location touches privacy requirements (REQ-N-009..011). |
| REQ-F-036, REQ-N-004, REQ-N-012..016 (country partitioning + full portability config) | Must | L | High | Country-abstraction approach itself is an open question (OQ-005); touches almost every module. |
| REQ-N-001, 002, 003 (speed, uptime, graceful degradation) | Must | L | High | Depends on the geospatial and push work above; measurement approach undefined (OQ-015). |
| REQ-N-006, 007, 008 (encryption, least privilege, security audit logging) | Must | M | Medium | "Encryption everywhere" is vague (OQ-010); otherwise standard practice. |
| REQ-N-009, 010, 011 (privacy: minimisation, retention, location opt-in) | Must | M | Medium | Retention period undefined (OQ-009); otherwise policy + implementation, not new tech. |
| REQ-F-037, 038, 039 (admin tools) | Must/Should | M | Laag | Standard admin CRUD once the underlying data models exist. |
...
- **CON-006** — FCM for push. Effect: fixes the push provider; OQ-004 (push reliability approach) is about how FCM is used, not which provider.
- **CON-007** — geo-indexed, per-country-partitioned datastore. Effect: **te beslissen nu** — the specific database/indexing technology (OQ-003) affects REQ-F-031, REQ-N-017, and the backend's Phase 1 CRUD endpoint should be built on a datastore that won't need replacing for the geospatial work in Phase 2.
- **CON-008** — four repos for a 2-dev + QA + BA team. Effect: confirms the Repo-tagged single-backlog approach used in 03-product-backlog-v1.md; e2e-tests repo excluded from this round (Section 5).
- **Te beslissen nu:** the country-abstraction technical approach (OQ-005) is worth deciding early even though it's Phase 2 work, since CON-003's module scaffold (Countries/Config) is being built in Phase 1 — deciding the abstraction shape now avoids reworking that module's boundary later.

## 9. Open scopingvragen

- **OQ-001 (tier breakdown)** — blocks a firm shape for REQ-F-015 (Phase 1, volunteer-app sign-up) and REQ-F-008/032 (Phase 2, tiered alerting). Phase 1 proceeds under the stated placeholder assumption (Section 3); Phase 2 cannot start the tiered-alert logic until this is resolved.
- **OQ-006 (navigation provider conflict)** — blocks a firm implementation for REQ-F-026 (Phase 1, volunteer-app navigate). Phase 1 proceeds under the stated assumption (Google Maps); if the diagram's OpenStreetMap/MapLibre/OSRM choice is confirmed instead, this story needs rework, not just a config change.
- **OQ-003 (geospatial database/indexing)** — blocks REQ-F-031/REQ-N-017 (Phase 2) and should ideally be decided before backend-api's Phase 1 CRUD endpoint's datastore choice is locked in, to avoid a migration.
- **OQ-005 (country-abstraction approach)** — blocks REQ-N-012..016 and REQ-F-032 (Phase 2), and shapes the Countries/Config module being scaffolded in Phase 1.
- **OQ-004 (push reliability approach)** — blocks REQ-F-021/022/033 (Phase 2) and the REQ-N-001 speed target's feasibility.
- **OQ-011 (role permission boundaries)** — blocks a precise scope for REQ-F-001 (Phase 2 login/roles).
- All other open questions carried from 01-requirements-structured-v1.md (OQ-002, OQ-007, OQ-008, OQ-009, OQ-010, OQ-012, OQ-013, OQ-014, OQ-015) affect Phase 2 or later items only and do not block Phase 1 as scoped above.
