...
|----|-------------|----------|--------|-------|
| REQ-F-001 | The dispatcher console shall support login with role-based access for three roles: dispatcher, supervisor, admin. | Must | Brief — "What the dispatcher console needs to do" | Exact permission boundaries per role are not stated — see OQ-011. |
...
| REQ-F-008 | The alert logic shall notify certified responders first (tiered order). | Must | Brief — "What the dispatcher console needs to do", "Goals" | Depends on final tier breakdown — see OQ-001. |
...
| REQ-F-015 | A volunteer shall be able to select/declare a training tier during sign-up. | Must | Brief — "What the volunteer app needs to do", "Volunteer tiers" | Final tier breakdown TBD — see OQ-001. |
...
| REQ-F-022 | Push alerts shall attempt to get through the device's silent/do-not-disturb mode, to the extent the platform and the user's settings allow. | Should | Brief — "What the volunteer app needs to do" | Feasibility/approach is an open question — see OQ-004. |
...
| REQ-F-026 | After accepting, a volunteer shall receive turn-by-turn navigation directions to the scene. | Must | Brief — "What we want to build", "What the volunteer app needs to do" | Provider conflicts with the architecture diagram — see OQ-006. |
...
| CON-007 | The backend's datastore includes geo-indexing and is partitioned per country. | Diagram ("Database — geo-index · per-country") | Implements REQ-F-036 / REQ-N-004; specific database technology not chosen — see OQ-003. |
...
- **OQ-001:** What is the final breakdown of volunteer tiers? The brief names three provisional categories (certified/verified CPR-BLS, healthcare professional, willing-but-untrained) but says the breakdown is "TBD" and that healthcare professional "might be its own tier, might be a flavour of certified." — affects REQ-F-008, REQ-F-015, REQ-F-032.
...
- **OQ-003:** What geospatial database and indexing approach will be used to keep nearby-volunteer search fast at country scale? Explicitly called out as undecided in the brief. — affects REQ-F-031, REQ-N-017, CON-007.
- **OQ-004:** What is the technical approach to maximise push delivery reliability (waking a backgrounded or silent Android device)? Explicitly called out as undecided in the brief. — affects REQ-F-021, REQ-F-022, REQ-N-001.
- **OQ-005:** How will country-specific differences (emergency number, regulations, language, integration partners, alert rules) be modelled technically without forking the codebase? Explicitly called out as undecided in the brief. — affects REQ-N-012 through REQ-N-016, REQ-F-032.
...
- **AS-001:** "Trained volunteers" in the brief's core flow description ("see which trained volunteers are nearby") is assumed to mean the same thing as the "certified" tier defined later in "Volunteer tiers," not a separate category. To be confirmed once OQ-001 is resolved. — affects REQ-F-008, REQ-F-032.
...
- **Dispatcher** — the 911/112 operator who uses the dispatcher web console to register an incident and alert volunteers. Brief also names supervisor and admin as further console roles, but their responsibilities beyond "dispatcher" are not detailed — see OQ-011.
...
- **Healthcare professional** — a possible separate volunteer tier, or a "flavour of certified" — undecided, see OQ-001.
