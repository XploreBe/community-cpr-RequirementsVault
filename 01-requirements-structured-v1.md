...
| REQ-F-008 | The alert logic shall notify certified responders first (tiered order). | Must | Brief — "What the dispatcher console needs to do", "Goals" | Tier breakdown confirmed — see OQ-001 [RESOLVED CHG-003]: certified/verified CPR-BLS, healthcare professional (separate tier), willing-but-untrained. AS-001 [RESOLVED CHG-010]: the first notified wave ("trained volunteers") is certified/verified CPR-BLS **and** healthcare professional together, not certified alone; willing-but-untrained is reached only via widening. |
...
| REQ-F-017 | The system shall track the expiry date of a volunteer's uploaded certification. | Must | Brief — "What the volunteer app needs to do" | OQ-014 resolved [CHG-017]: expired certification excludes the volunteer from new alerts until re-verified — no automatic tier demotion. |
| REQ-F-018 | The system shall remind a volunteer to re-verify their certification (e.g. ahead of expiry). | Should | Brief — "What the volunteer app needs to do" | Timing/channel of reminder not specified. OQ-014 resolved [CHG-017] — see REQ-F-017. |
...
| REQ-F-021 | The volunteer app shall receive a push alert when the volunteer is identified as a nearby responder for an incident. | Must | Brief — "What we want to build", "What the volunteer app needs to do" | OQ-012 resolved [CHG-013]: offline/no-signal/force-closed counts as "not reached" — no retry/queueing logic, widening proceeds as if ignored. |
...
| REQ-F-024 | A volunteer shall be able to accept an alert with one tap. | Must | Brief — "What we want to build", "Goals", "What the volunteer app needs to do" | OQ-013 resolved [CHG-014]: no back-out after accepting — once accepted, accepted, no back-out flow in this round. |
...
| REQ-F-032 | The backend shall run the tiered alert logic, with the tiering/ordering/distance rules configurable per country. | Must | Brief — "What the backend needs to do", "Volunteer tiers" | AS-001 [RESOLVED CHG-010]: first-wave "trained volunteers" = certified/verified CPR-BLS + healthcare professional combined, not certified alone. |
...
| REQ-F-035 | The backend shall hold and maintain the volunteer registry: tier, certification, status, and history. | Must | Brief — "What the backend needs to do" | OQ-014 resolved [CHG-017]: an in-flight incident is never interrupted — a notified/accepted volunteer stays active on that incident regardless of a certification change or deactivation that happens mid-response. |
...
| REQ-N-006 | The system shall encrypt data "everywhere" (source's own wording). | Security | Must | Brief — "Non-functional stuff" (Security) | OQ-010 resolved [CHG-016]: minimum HTTPS/TLS in transit everywhere, plus at-rest encryption for sensitive fields via the chosen datastore's defaults; no specific key-management scheme mandated, delegated to the dev team. |
...
| REQ-N-010 | Patient location data shall not be retained longer than necessary. | Privacy | Must | Brief — "Non-functional stuff" (Privacy) | OQ-009 resolved [CHG-012]: 90 days, placeholder value pending real legal review per deployment jurisdiction. |
...
| REQ-N-017 | Nearby-volunteer search shall return results in sub-second time, including in dense cities. | Performance | Must | Brief — "What the backend needs to do" | OQ-007 resolved [CHG-011]: kept simple — flat "under 1 second," no concurrency/region/volunteer-count benchmark specified. Related functional requirement: REQ-F-031. |
| REQ-N-018 | Background location tracking on the volunteer app shall be battery-friendly. | Performance/Usability | Should | Brief — "What the volunteer app needs to do" | OQ-008 resolved [CHG-015]: no concrete number — "reasonable battery use, no aggressive constant GPS polling," left to dev discretion. |
...
- ~~**OQ-007:** What precisely does "sub-second" mean for nearby-volunteer search — under what concurrency, region size, and volunteer-count assumptions should this be benchmarked?~~ — **RESOLVED [CHG-011]:** kept simple — a flat "under 1 second," no concurrency/region/volunteer-count benchmark specified. — affects REQ-N-017.
- ~~**OQ-008:** What precisely does "battery-friendly" mean for background location tracking (e.g. a target sampling interval or battery-drain budget)?~~ — **RESOLVED [CHG-015]:** no concrete number specified — "reasonable battery use, no aggressive constant GPS polling," left to the development team's discretion. — affects REQ-N-018.
- ~~**OQ-009:** What is the precise retention period for patient location data once an incident is closed?~~ — **RESOLVED [CHG-012]:** 90 days, as a placeholder value — easy to change later, not a legally-researched figure (this is a test project; GDPR itself sets no fixed number, only a "no longer than necessary" principle — Art. 5(1)(e)). — affects REQ-N-010.
- ~~**OQ-010:** What precisely does "encryption everywhere" cover — data in transit only, data at rest, specific fields, and what key-management approach?~~ — **RESOLVED [CHG-016]:** minimum HTTPS/TLS in transit for everything, plus at-rest encryption for sensitive fields (location, personal data) using whatever the chosen datastore offers by default — no specific key-management scheme mandated, left to the development team. — affects REQ-N-006.
...
- ~~**OQ-012:** What should happen if a volunteer's device is offline, has no signal, or the app has been force-closed by the OS when an alert is sent?~~ — **RESOLVED [CHG-013]:** counts simply as "not reached" — no retry/queueing logic; the alert-widening flow proceeds exactly as if the volunteer had ignored the alert. — affects REQ-F-021, REQ-N-001.
- ~~**OQ-013:** Can a volunteer back out after already accepting an alert (as opposed to declining before accepting), and if so how is that handled downstream?~~ — **RESOLVED [CHG-014]:** no — once accepted, accepted. No back-out flow in this round. — affects REQ-F-024, REQ-F-010.
- ~~**OQ-014:** What happens when a volunteer's certification expires without re-verification, and what happens to an in-flight incident if a notified/accepted volunteer's account is deactivated mid-response?~~ — **RESOLVED [CHG-017]:** an expired certification excludes the volunteer from new alerts until re-verified — no automatic demotion to a lower tier. An in-flight incident is never interrupted: a volunteer already notified/accepted stays active on that incident regardless of a certification change or deactivation that happens mid-response. — affects REQ-F-017, REQ-F-018, REQ-F-035.
...
- ~~**AS-001:** "Trained volunteers" in the brief's core flow description ("see which trained volunteers are nearby") is assumed to mean the same thing as the "certified" tier defined later in "Volunteer tiers," not a separate category.~~ — **RESOLVED [CHG-010]:** the assumption was too narrow. "Trained volunteers" (the first alert wave) means certified/verified CPR-BLS **and** healthcare professional combined; willing-but-untrained is reached only if the alert widens. — affects REQ-F-008, REQ-F-032.
