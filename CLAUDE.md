...
- **Keep stories simple and purely functional — and default small.** Ground every story in what the requirements actually say, but "a REQ-xxx somewhere calls for it" is NOT by itself a reason to build it in this round. A requirement can be true and still belong in a later phase. Concretely: unless Mohamed explicitly asks for it, do not include auth/roles/MFA, security infrastructure (encryption scope, audit logging, least-privilege enforcement), i18n/multi-country configurability, or admin tooling in a "keep it simple" backlog round — bucket these into a later phase instead, even though they're real, valid, Must-priority requirements. Default to the smallest slice that delivers the actual user-facing capability (e.g. "volunteer sets availability and it's respected" — not "volunteer logs in with MFA, sets a country-aware availability preference, which an admin can audit"). When genuinely unsure whether something is core or an add-on layer, ask before including it — don't reason your way into "it's grounded, so it's in."
